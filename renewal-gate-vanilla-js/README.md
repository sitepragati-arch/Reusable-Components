# renewal-gate-vanilla-js

A portable "paywall" that locks a site behind a SitePragati renewal/payment
check — no framework required. This is the vanilla-JS counterpart to
[`renewal-gate/`](../renewal-gate) for projects that don't use React — a
static site, a server-rendered site, or anything served from Cloudflare
Pages (or an equivalent host with server-side functions).

Two files, one `<script>` tag. No build step, no npm dependency, no
client-side QR library.

> **Different backend than `renewal-gate/renewal-service`.** This variant
> talks directly to SitePragati's own public customer-info API
> (`https://sitepragati.in/api/public/customer-info`) — not the shared
> Google Sheet / `renewal-service` Worker the React component uses. The two
> are not interchangeable and don't share data. Pick one pattern per
> project: React app → [`renewal-gate/`](../renewal-gate); anything else →
> this one.

**Reference integration:** the jewelry-shop project (Cloudflare Pages + D1,
plain HTML/CSS/JS storefront + Admin Console + Vendor Portal) is the first
app wired up this way — see its `docs/renewal-gate.md` for the same content
in that project's own context, and `public/js/renewal-gate.js` +
`functions/api/renewal-status.js` for the actual running code.
...
| File | Runs where | Job |
|---|---|---|
| `functions/api/renewal-status.js` (path/convention varies per host) | Server | Same-origin proxy in front of SitePragati's public API |
| `public/js/renewal-gate.js` (path varies per project) | Browser | Fetches the proxy, decides overdue-or-not, renders the lock screen |
...
the jewelry-shop integration initially was. Check headers on a real `GET`,
not `HEAD`.) A browser calling that URL directly with `fetch()` gets blocked
by CORS, full stop — you'll see this in the console:
...
- There is **no QR-image endpoint** on this API (unlike `renewal-service`,
  which does have one) — build the UPI QR code yourself from `upi_id` +
  `next_payment_due_amount` (see below).
- `customerId` is whatever id SitePragati's side has assigned this
  project/client. Provisioning a new customer record is a manual step on
  SitePragati's side — this API is read-only from the outside.
...
`ok`/`error` above are tiny JSON-response helpers a host project may or may
not already have. If not, the whole handler is just as easily a plain
`Response`:
...
> The jewelry-shop reference integration additionally reads `CUSTOMER_ID`
> from that project's own vendor-editable settings table (falling back to a
> hardcoded value) — that's a project-specific enhancement, not part of the
> minimal pattern. See **Optional: vendor-configurable customer ID** below
> if you want that too.

## Step-by-step: adding this to a new project

1. **Copy both files** into the new project:
   - `functions/api/renewal-status.js` (or the equivalent path/convention
     for that project's backend — see the plain-`Response` version above if
     it doesn't have Cloudflare Pages Functions at all)
...
2. **Set `CUSTOMER_ID`** in `renewal-gate.js` to a fresh id for this new
   ...
   (`sitepragati.in`) to create that customer record first — this only
...
3. **Load the script on every page that should be gated**, as early as
   reasonably possible in `<body>` (the jewelry-shop integration puts it as
   the very first `<script>` tag, before anything else, on every page —
   storefront, admin, and vendor portal alike):
   ```html
...
- **Fails open, deliberately.** A network error, a missing field, an
  ...
  either SitePragati's side or the proxy's. If a project needs it to fail
  *closed* instead (lock by default on any doubt), that's a one-line change
  in `run()` — but think hard before making that change, since a truly
  broken proxy would then lock every gated project at once.
...
- **The QR code is generated by a third-party public image API**
  ...
  to distribute, but it means QR generation depends on that service's
  uptime. Swap the URL builder in `buildUpiQrUrl()` for a different provider
  (or a bundled QR library) if that's ever a concern.
- **The proxy is intentionally dumb** — it doesn't cache, doesn't transform
  ...
  Keep it that way unless there's a specific reason not to (e.g.
  rate-limiting many gated projects hitting SitePragati at once — not a
  problem hit yet).

## Optional: vendor-configurable customer ID

The jewelry-shop reference integration additionally lets whoever manages
that site change `CUSTOMER_ID` from a settings screen instead of it being
hardcoded — useful if the same codebase template gets reused for a
different client under a different SitePragati account, since that becomes
a one-field change instead of a code edit and redeploy. If a new project has
an equivalent "site settings" concept already, the same idea ports over:
...
`CUSTOMER_ID` (the minimal version above) is a perfectly fine starting point
for a new project, and this can be added later without changing anything
else.
